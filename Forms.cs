...
namespace Shop
{
    public partial class Forms : Form
    {
        ...
        private void prodaddbtn_Click(object sender, EventArgs e)
        {
            try
            {
                if (prodname.Text == "" || quantity.Text == "" || price.Text == "" || cost_price.Text == "" || category.Text == "" || ingredients.Text == "")
                {
                ...
                {
                    Con.Open();
                    string query = "INSERT INTO ProdTable (ProdName, Quantity, Price, CostPrice, Category, Ingredients) " +
                                   "VALUES (@ProdName, @Quantity, @Price, @CostPrice, @Category, @Ingredients)";

                    SqlCommand command = new SqlCommand(query, Con);
                    command.Parameters.AddWithValue("@ProdName", prodname.Text);
                    command.Parameters.AddWithValue("@Quantity", Convert.ToInt32(quantity.Text));
                    command.Parameters.AddWithValue("@Price", Convert.ToDecimal(price.Text));
                    command.Parameters.AddWithValue("@CostPrice", Convert.ToDecimal(cost_price.Text));
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            ingredients.Text = prodList.SelectedRows[0].Cells[5].Value.ToString();
